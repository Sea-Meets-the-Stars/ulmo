...
def load_ulmo_model(model_name:str, datadir=None, local=False):
    ...
    elif model_name == 'swot-prototype':
        if datadir is None:
            if local:
                raise IOError("No local storage for SWOT prototype")
            else:
                datadir = os.path.join('s3://ssh', 'Models', 'SWOT_Proto_std')
        filepath = 'PreProc/swot_prototype_train.h5'
    else:
